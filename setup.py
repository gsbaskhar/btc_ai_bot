...
# Create __init__.py
with open('strategy/__init__.py', 'w') as f:
    f.write('''# strategy/__init__.py
from .enhanced_strategy import *
''')

# Create enhanced_strategy.py
with open('strategy/enhanced_strategy.py', 'w') as f:
    f.write('''# strategy/enhanced_strategy.py
import pandas as pd
import numpy as np
from config import *
...
def get_trend(df):
    ...
    # MACD
    bullish_macd = last["macd"] > last["macd_signal"] and last["macd"] > 0
    bearish_macd = last["macd"] < last["macd_signal"] and last["macd"] < 0
    
    # RSI
    bullish_rsi = last["rsi"] > 50
    bearish_rsi = last["rsi"] < 50
...
def get_breakout_signal(df, trend, debug=False):
    ...
    strong_candle = body >= avg_body * 0.7
